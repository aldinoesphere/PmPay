...
* @package PmPay\Services\Database
*/
class SettingsService extends DatabaseBaseService
{
    ...
    public function getConfiguration($plentyId, $settingType)
    {
        ...
        $url = $paymentHelper->getDomain() . '/rest/payment/pmpay/setting/' . $plentyId . '/' . $settingType;
        ...
        if (curl_errno($curl))
        {
            $this->getLogger(__METHOD__)->error('PmPay:error', curl_error($curl));
    ...
    /**
     * save PmPay configuration to database
     *
    ...
    public function saveConfiguration($parameters)
    {
        ...
        $url = $paymentHelper->getDomain() . '/rest/payment/pmpay/settings/';
        $header = array(
